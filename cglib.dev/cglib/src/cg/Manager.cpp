...
namespace cg {
	...
	void appShutdownCallback(int value) {
		Application *app = Manager::instance()->getApp(); 
    ...
    void appUpdateCallback(int value) {
		if(Manager::instance()->_isShutdown) {
		    glutTimerFunc(Manager::instance()->getIntervalMillis(),appShutdownCallback,0);
		} else {
