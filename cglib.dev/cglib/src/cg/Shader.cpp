...
namespace cg {
	...
	bool Shader::loadAndCompile(const std::string& filename) {
		const std::string shader_string = read(filename);
		const GLchar* shader_source = shader_string.c_str();
		GLint shader_length = shader_string.size();
		assert(shader_source != 0);
		glShaderSource(_shaderId, 1, &shader_source, &shader_length);
