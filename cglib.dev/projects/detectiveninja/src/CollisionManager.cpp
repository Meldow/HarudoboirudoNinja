...
namespace proj {
	...
	void CollisionManager::update(unsigned long elapsed_millis) {
		if (getListsSize() > 0) {
			...
			for (int clueIdx = 0; clueIdx < (int)_clues->size(); clueIdx++) {
			...
			for (int sexyMagIdx = 0; sexyMagIdx < (int)_sexyMagazines->size(); sexyMagIdx++) {
			...
			for (int staticObjIdx = 0; staticObjIdx < (int)_staticObjects->size(); staticObjIdx++) {
			...
			for (int enemyIdx = 0; enemyIdx < (int)_enemies->size(); enemyIdx++) {
				...
				if (enemyCollidingBSIdx > -1) {	// Ninja lost !
					...
					static_cast<Enemy*>(enemy)->spottedSexyMagazine(false);
					static_cast<Enemy*>(enemy)->spottedNinja();
					static_cast<Scene*>(cg::Registry::instance()->get("Scene"))->onLose();
					static_cast<MenuManager*>(cg::Registry::instance()->get("MenuManager"))->goToMenuByName("LoseMenu");
	...
	bool CollisionManager::heightMapHorizontalCollision(Collidable* dynamicObject) {
		...
		if ( (_map->Height(	(int)(objPosition[0] + objDirection[0]*dynamicObject->getBoundingSphere(0)->getRadius()*2), (int)(objPosition[1] + objDirection[1]*dynamicObject->getBoundingSphere(0)->getRadius()*2)) 
				> (int)(objPosition[2] + objDirection[2]*dynamicObject->getBoundingSphere(0)->getRadius()*2+10)) 
				||
				_map->Height((int)(objPosition[0] - objDirection[0]*dynamicObject->getBoundingSphere(0)->getRadius()*2), (int)(objPosition[1] - objDirection[1]*dynamicObject->getBoundingSphere(0)->getRadius()*2)) 
		> (int)(objPosition[2] + objDirection[2]*dynamicObject->getBoundingSphere(0)->getRadius()*2+10)) {
	...
	void CollisionManager::heightMapVerticalCollision(Collidable* dynamicObject) {
		...
		float fracPosX = (float) (objPosition[0] - roundPosX);
		float fracPosY = (float) (objPosition[1] - roundPosY);
		...
		dynamicObject->setHeight((int)avgHeight);
	}
	...
	int CollisionManager::ninjaVsEnemyCollisionDetection(Collidable* ninja, Collidable* enemy) {
		...
		for (int ninjaBSIdx = 0; ninjaBSIdx < (int)ninja->getBoundingSpheres()->size(); ninjaBSIdx++) {

			for (int enemyBSIdx = 0; enemyBSIdx < (int)enemy->getBoundingSpheres()->size(); enemyBSIdx++) {
	...
	int CollisionManager::dynamicVsStaticObjectCollisionDetection(Collidable* dynamicObject, Collidable* staticObject) {
		...
		for (int dynamicObjectBSIdx = 0; dynamicObjectBSIdx < (int)dynamicObject->getBoundingSpheres()->size(); dynamicObjectBSIdx++) {
