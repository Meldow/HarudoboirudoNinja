...
namespace proj {
	...
	void CameraMode::limitPosition() {
		
		if(_position[0] < MIN_CAM_X)
			_position[0] = MIN_CAM_X;
		if(_position[1] < MIN_CAM_Y)
			_position[1] = MIN_CAM_Y;
		if(_position[2] < MIN_CAM_Z)
			_position[2] = MIN_CAM_Z;
		if(_position[0] > MAX_CAM_X)
			_position[0] = MAX_CAM_X;
		if(_position[1] > MAX_CAM_Y)
			_position[1] = MAX_CAM_Y;
		if(_position[2] > MAX_CAM_Z)
			_position[2] = MAX_CAM_Z;
	}

	void CameraMode::reset() {
	}
