...
namespace proj {
	...
	private:
		...
		cg::Vector3d _finalPosition, _initialPosition,_front;
		float _time, _frontspeed, _downspeed, _rotation;
		bool _throwing, _throwed;
	...
	public:
		...
		float setPhysicPosition(float time);

		void reset();
		void move(unsigned long elapsed_millis);
	};
}
