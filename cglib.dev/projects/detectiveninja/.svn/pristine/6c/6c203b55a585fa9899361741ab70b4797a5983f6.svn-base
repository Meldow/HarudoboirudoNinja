...
namespace proj {
	...
	void ImageLoaderHeightMap::setup() {
		...
		dynamic_cast<DebugManager*>(cg::Registry::instance()->get("DebugManager"))->registerMap(dynamic_cast<Debug*>(this));
		// LOAD TEXTURES
		tm = dynamic_cast<TextureManager*>( cg::Registry::instance()->get("TextureManager"));
		txHeightMap = tm->get(tm->getMapTexName())->getTextureDL();
	}
				...
				for ( X = 0; X < MAP_SIZE; X += STEP_SIZE )
					for ( Y = 0; Y < MAP_SIZE; Y += STEP_SIZE )
					{
						...
						glBegin(GL_LINES);
							...
							glVertex3f((GLfloat)(x+normalvec[0]), (GLfloat)(y+normalvec[1]), (GLfloat)(z+normalvec[2]));
						glEnd( );
					}
					glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);	
		}
